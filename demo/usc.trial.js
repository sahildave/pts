...
(function() {
	var run = Pts.quickStart("#pt", "#ffff00");
	run((time, ftime) => {
        ...
        var uOuter = Polygon.convexHull([p1, p2, pointer, p6]);
        uOuter.insert( uOuter.slice(0, 3), uOuter.length );
		form.fill(Color.from(255, 0, 0, alpha).toString("rgba")).polygon(Curve.bspline(uOuter , 10, 0.25));
        
		form.fill(Color.from(255, 255, 0, alpha).toString("rgba")).polygon([p2, p3, p4, pointer]);
		
        var cOuter = Polygon.convexHull([pointer, p4, p5, p6]);
        cOuter.insert( cOuter.slice(0, 3), cOuter.length );
        form.fill(Color.from(0, 255, 0, alpha).toString("rgba")).polygon(Curve.bspline(cOuter , 10, 0.25));
		...
		var uGroup = Polygon.convexHull(Group.fromPtArray([uPointsTop[0], uPointsTop[1], uPointsCenter[1], uPointsCenter[0]]));
        // var uGroup = Group.fromPtArray([uPointsTop[0], uPointsTop[1], uPointsCenter[1], uPointsCenter[0]]);
		uGroup.insert( uGroup.slice(0, 3), uGroup.length );
        form.fillOnly("rgb(255,255,0)").polygon(Curve.bspline( uGroup, 10, 0.25));
		...
		var cGroupMidTop = Group.fromPtArray([cPointsLeft[0], sPointsShapeEnd[0]]);
		var cGroupMidBottom = Group.fromPtArray([cPointsLeft[1], sPointsShapeEnd[1]]);
		var cPointsCenter = Group.fromPtArray([cGroupMidTop.interpolate(0.25), cGroupMidBottom.interpolate(0.25)]);
		form.fill("#123").point(sPointsRight[0]).point(sPointsShapeMid[0]);
		form.fill("#123").point(sPointsShapeEnd[0]).point(cPointsCenter[0]);
		form.fill("#123").point(cPointsCenter[1]).point(sPointsShapeEnd[1]);
		form.fill("#123").point(sPointsShapeCenter[0]).point(sPointsRight[1]);
		// var cGroup = Group.fromPtArray([cPointsCenter[0], cGroupMidTop[1], cGroupMidBottom[1], cPointsCenter[1]]);
		
        var scGroup = Group.fromPtArray([sPointsRight[0], sPointsShapeMid[0], sPointsShapeEnd[0],cPointsCenter[0], 
                                        cPointsCenter[1], sPointsShapeEnd[1], sPointsShapeCenter[0], sPointsRight[1]]);
        var scShape = Polygon.convexHull(scGroup);
		scShape.insert( scShape.slice(0, 3), scShape.length );
        // form.fillOnly("rgb(255,0,0)").polygon(Curve.bspline( scShape));
        // form.fillOnly("rgb(255,0,0)").polygon(scGroup);
