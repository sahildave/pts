...
(function() {
    ...
    space.add({  
        ...
        animate: (time, ftime) => {
            ...
            var timer = (Num.cycle(time%3000/3000) * 0.2);
            circleSize = baseCircleSize + (baseCircleSize * (Num.cycle(time%2000/2000) * 0.5));
            // form.fill("#fff").font(24, "bold").text(new Pt({ x: 0, y: 50}), time); 
            // form.fill("#fff").font(24, "bold").text(new Pt({ x: 0, y: 100}), timer); 
            createCircle(KEY_SLANT_TOP, newDir, true,  (magnitudeMultiplier+(refMag*1*magnitudeMultiplier)/spaceMag));
            centerCircles = createCircle(KEY_HORIZ, dirHoriz, false, (magnitudeMultiplier+(refMag*1.5*magnitudeMultiplier)/spaceMag));
            createCircle(KEY_SLANT_BOTTOM, newDir, false,  (magnitudeMultiplier+(refMag*1*magnitudeMultiplier)/spaceMag));
