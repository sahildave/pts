...
(function() {
    ...
    function switchBG(space, time) {
        var bgVariants = [ "rgb(225, 219, 57)", "rgb(252, 114, 70)", "rgb(142, 47, 15)",
            "rgb(141, 210, 219)", "rgb(63, 108, 114)", "rgb(158, 147, 167)",
            "rgb(59, 56, 59)"];
        var sectionWidth = space.width / bgVariants.length;

        var index = Math.floor((space.pointer.x/sectionWidth));
        var bgColor = bgVariants[index];
        console.log("BG - "+index);
        form.fill("#fff").font(16, "bold").text(new Pt({ x: 0, y: 50}), "BG , "+space.pointer.x+", "+sectionWidth+", "+index);
        space.background = bgColor;
    }
    ...
    space.add({  
        ...
        animate: (time, ftime) => {
            ...
            var uCoordinates = new Pt({x: topCircles[2][0].x,    y: topCircles[0][0].y});
            var cCoordinates = new Pt({x: bottomCircles[0][0].x, y: bottomCircles[2][0].y});

            var uRect = Rectangle.fromCenter(uCoordinates, baseCircleSize*3);
            var uFontSize = Typography.fontSizeToBox( uRect, 0.75 )(uRect);
 
            form.font(uFontSize).fill(colorFirst).alignText("center").textBox( uRect, "U", "top", "..." );

            var cRect = Rectangle.fromCenter(cCoordinates, baseCircleSize*3);
            var cFontSize = Typography.fontSizeToBox( cRect, 0.75 )(cRect);
            form.font(cFontSize).fill(colorFourth).alignText("center").textBox( cRect, "C", "bottom", "..." );
        }
    });
